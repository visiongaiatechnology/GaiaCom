...
export default function AbuseCenter({ activeIdentity, triggerAlert, t }) {
	...
	const [disclosureRole, setDisclosureRole] = useState('reporter');
	const [disclosurePackageText, setDisclosurePackageText] = useState('{\n  "type": "GaiaProofDisclosure",\n  "version": "v1",\n  "statement": ""\n}');
	const [gaiasEyesTTL, setGaiasEyesTTL] = useState(900);
	const [gaiasEyesKey, setGaiasEyesKey] = useState(null);
	const [requestTargetRole, setRequestTargetRole] = useState('reporter');
	const [requestReason, setRequestReason] = useState('Bitte freiwillig ein fallbezogenes GaiaProof-Paket zur Prüfung freigeben.');
	...
	const parseDisclosurePackage = () => {
		const parsed = safeJsonParse(disclosurePackageText, null);
		if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') {
			throw new Error('Disclosure-Paket muss ein JSON-Objekt sein.');
		}
		return parsed;
	};
	...
	const refreshSelectedCase = async () => {
		...
		const res = await api.getReportDetail(selectedCase.id);
		setCaseDetail(res);
	};

	const handleDisclosureSubmit = async (e) => {
		e.preventDefault();
		if (!activeIdentity || !selectedCase) return;
		setBusy(true);
		try {
			const disclosurePackage = parseDisclosurePackage();
			await api.createCaseDisclosure(selectedCase.id, activeIdentity.ID, disclosureRole, disclosurePackage, '');
			triggerAlert('Disclosure gespeichert', 'Das freiwillige Beweispaket wurde dem Fall hinzugefügt.');
			await refreshSelectedCase();
		} catch (err) {
			triggerAlert('Fehler', err.message || 'Disclosure fehlgeschlagen.', 'danger');
		} finally {
			setBusy(false);
		}
	};

	const handleCreateGaiasEyesKey = async () => {
		if (!activeIdentity || !selectedCase) return;
		setBusy(true);
		try {
			const disclosurePackage = parseDisclosurePackage();
			const res = await api.createGaiasEyesGrant(activeIdentity.ID, {
				caseId: selectedCase.id,
				scope: 'case',
				disclosurePackage,
				signature: '',
				ttlSeconds: Number(gaiasEyesTTL) || 900
			});
			setGaiasEyesKey(res.key || null);
			triggerAlert('GaiasEyesKey erstellt', 'Der Einmal-Schlüssel wurde erzeugt und zerfällt nach Nutzung.');
			await refreshSelectedCase();
		} catch (err) {
			triggerAlert('Fehler', err.message || 'GaiasEyesKey konnte nicht erstellt werden.', 'danger');
		} finally {
			setBusy(false);
		}
	};
	...
	const handleRequestDisclosure = async () => {
		...
		setBusy(true);
		try {
			await api.requestCaseDisclosure(selectedCase.id, activeIdentity.ID, requestTargetRole, requestReason);
			triggerAlert('Anfrage gesendet', 'Die Offenlegungsanfrage wurde im Fallprotokoll vermerkt.');
			await refreshSelectedCase();
		} catch (err) {
			triggerAlert('Fehler', err.message || 'Offenlegungsanfrage fehlgeschlagen.', 'danger');
		} finally {
			setBusy(false);
		}
	};
	...
	const renderDisclosurePanel = () => {
		const disclosures = caseDetail?.disclosures || [];
		const disclosureRequests = caseDetail?.disclosureRequests || [];
		const canRequestDisclosure = isReviewer || isOperator;
		return (
			<div className="proof-comment-box">
				<h4>GaiasEyesKey / Mutual Disclosure</h4>
				<p className="comment-text">
					Freiwillige, fallbezogene Beweispakete. Kein Account-Zugang, kein Senden, kein Löschen.
				</p>
				{disclosures.length > 0 && (
					<div className="existing-reviews">
						<h4>Freigegebene Pakete ({disclosures.length})</h4>
						{disclosures.map(item => (
							<div key={item.id} className="reviewer-vote-row">
								<span>{item.disclosureRole} · {item.ownerIdentity}</span>
								<strong>{item.packageHash}</strong>
							</div>
						))}
					</div>
				)}
				{disclosureRequests.length > 0 && (
					<div className="existing-reviews">
						<h4>Offenlegungsanfragen ({disclosureRequests.length})</h4>
						{disclosureRequests.map(item => (
							<div key={item.id} className="reviewer-vote-row">
								<span>{item.targetRole} · {item.status}</span>
								<strong>{item.reason}</strong>
							</div>
						))}
					</div>
				)}
				{canRequestDisclosure && (
					<div className="case-audit-card">
						<div className="form-row-2">
							<div className="form-group">
								<label>Anfrage an</label>
								<select className="input-field" value={requestTargetRole} onChange={e => setRequestTargetRole(e.target.value)}>
									<option value="reporter">Melder</option>
									<option value="reported">Gemeldeter</option>
									<option value="owner">Owner</option>
									<option value="authority">Behörden-/Rechtsstelle</option>
								</select>
							</div>
							<div className="form-group">
								<label>Anfragegrund</label>
								<input
									type="text"
						...
						<button type="button" className="btn-secondary" disabled={busy} onClick={handleRequestDisclosure}>
							Offenlegung anfragen
						</button>
					</div>
				)}
				<form onSubmit={handleDisclosureSubmit}>
					<div className="form-row-2">
						<div className="form-group">
							<label>Disclosure-Rolle</label>
							<select className="input-field" value={disclosureRole} onChange={e => setDisclosureRole(e.target.value)}>
								<option value="reporter">Melder</option>
								<option value="reported">Gemeldeter</option>
								<option value="owner">Eigener Nachweis</option>
								<option value="authority">Behörden-/Rechtsnachweis</option>
								<option value="other">Sonstiges</option>
							</select>
						</div>
						<div className="form-group">
							<label>GaiasEyesKey TTL</label>
							<select className="input-field" value={gaiasEyesTTL} onChange={e => setGaiasEyesTTL(Number(e.target.value))}>
					...
					<div className="form-group">
						<label>Signiertes JSON-Beweispaket</label>
						<textarea
							className="input-field"
							value={disclosurePackageText}
							onChange={e => setDisclosurePackageText(e.target.value)}
							spellCheck="false"
							rows={8}
						/>
					</div>
					<div className="form-row-2">
						<button type="submit" className="btn-secondary" disabled={busy}>
							Disclosure dem Fall anhängen
						</button>
						<button type="button" className="btn-primary" disabled={busy} onClick={handleCreateGaiasEyesKey}>
							GaiasEyesKey erzeugen
						</button>
					</div>
				</form>
				{gaiasEyesKey && (
					<div className="case-audit-card">
						<div><span>Typ:</span> <strong>{gaiasEyesKey.type}</strong></div>
						<div><span>Gültig bis:</span> <strong>{formatTime(gaiasEyesKey.expiresAt)}</strong></div>
						<div><span>Einmal-Token:</span> <code>{gaiasEyesKey.token}</code></div>
					</div>
				)}
			</div>
		);
						...
						<div className="abuse-detail-pane glass-panel">
							{selectedCase ? (
								<>
									...
									{caseDetail && renderDisclosurePanel()}
