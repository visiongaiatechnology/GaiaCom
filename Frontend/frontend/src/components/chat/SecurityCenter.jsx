  ...
  return (
    <div className="security-center-pane gaia-scrollbar" style={{ padding: '24px', overflowY: 'auto', height: '100%' }}>
      ...
      <div className="security-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <span className="auth-kicker">{t('sc_subtitle') || 'GaiaShield Integrated Security Layer'}</span>
          <h1 style={{ margin: '4px 0 0 0', fontSize: '1.8rem', fontWeight: '800' }}>{t('sc_title') || 'Security Center'}</h1>
        </div>
      </div>
      ...
      <div className="security-tabs" style={{ display: 'flex', gap: '8px', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px', marginBottom: '24px' }}>
        ...
        >
          {t('sc_tab_passport') || 'Gaia Passport'}
        </button>
        ...
        {isOperator && (
          ...
          >
            {t('sc_tab_node_system') || 'Nodesystem'}
          </button>
        )}
      </div>
              ...
              <div className="security-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
                ...
                <div className="security-card glass-panel" style={{ padding: '20px', borderRadius: '12px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    ...
                    <strong style={{ color: 'var(--success)' }}>{t('sc_optimal') || 'Optimal'}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div className="security-events-tab">
              <div className="filter-bar glass-panel" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderRadius: '12px', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <div>
                    ...
                    <select className="input-field compact-input" value={filterSeverity} onChange={(e) => setFilterSeverity(e.target.value)}>
                      ...
                      <option value="info">{t('sc_opt_info') || 'Info'}</option>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn-secondary compact-btn" onClick={() => handleExport('json')}>{t('sc_btn_download_json') || 'Download JSON'}</button>
                  <button className="btn-secondary compact-btn" onClick={() => handleExport('csv')}>{t('sc_btn_download_csv') || 'Download CSV'}</button>
                </div>
              </div>
            ...
            <div className="security-passport-tab">
              <div className="security-passport-grid">
                ...
                <section className="glass-panel security-passport-proof-panel">
                  <span className="auth-kicker">{t('sc_human_proof_poc') || 'Human Proof PoC'}</span>
                  <h3>{t('sc_sha256_ceremony') || 'SHA-256 Ceremony'}</h3>
                  <p>
                    {t('sc_ceremony_desc') || 'Die Ceremony laeuft lokal im Browser-Worker, wird mit deinem Identitaetsschluessel signiert und nach Serverpruefung im Trust Passport gespeichert.'}
                  </p>
                  <div className="security-passport-proof-list">
                    <div><strong>{t('sc_col_status') || 'Status'}</strong><span>{humanProof ? (t('sc_verified') || 'Verifiziert') : (t('sc_not_verified') || 'Nicht verifiziert')}</span></div>
                    <div><strong>{t('sc_algorithm') || 'Algorithmus'}</strong><span>{t('sc_sha256_pow_desc') || 'SHA-256 chained proof-of-work'}</span></div>
                    <div><strong>{t('sc_signature') || 'Signatur'}</strong><span>{t('sc_ed25519_desc') || 'Ed25519 Identity Key'}</span></div>
                    <div><strong>{t('sc_scope') || 'Scope'}</strong><span>{t('sc_scope_desc') || 'Node-gespeicherter Gaia Passport'}</span></div>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    {humanProof ? (t('sc_btn_renew_human_proof') || 'Human Proof erneuern') : (t('sc_btn_verify_human') || 'Ich bin ein Mensch verifizieren')}
                  </button>
                </section>
              </div>
            </div>
            ...
            <div className="security-node-system-tab" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <section className="glass-panel" style={{ padding: '22px', borderRadius: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  <div>
                    <span className="auth-kicker">{t('sc_fed_registry_title') || 'GaiaCom Federation Registry'}</span>
                    <h3 style={{ margin: '6px 0 8px 0' }}>{t('sc_node_system_title') || 'Nodesystem'}</h3>
                    <p style={{ margin: 0, color: 'var(--text-secondary)', maxWidth: '760px' }}>
                      {t('sc_fed_registry_desc') || 'Dieser Node kann seine S2S-Secrets erzeugen, den Registry-Mainnode pingen und verbundene Nodes anhand von Public Key, Core-Hash und Status verwalten.'}
                    </p>
                  </div>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <button type="button" className="btn-secondary compact-btn" onClick={handleGenerateNodeSecrets}>
                      {t('sc_btn_generate_secrets') || 'Secrets generieren'}
                    </button>
                    <button type="button" className="btn-primary compact-btn" onClick={handlePingMainNode}>
                      {t('sc_btn_ping_main') || 'Main-Node pingen'}
                    </button>
                  </div>
                </div>
              </section>
              ...
              {nodeRegistryError && (
                <section className="glass-panel" style={{ padding: '16px', borderRadius: '12px', borderLeft: '4px solid var(--warning)' }}>
                  <strong style={{ color: 'var(--warning)' }}>{t('sc_registry_notice_title') || 'Registry Hinweis'}</strong>
                    ...
                    <table className="gaia-table security-events-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>
                          <th style={{ padding: '12px' }}>{t('sc_col_node') || 'Node'}</th>
                          <th style={{ padding: '12px' }}>{t('sc_col_status') || 'Status'}</th>
                          <th style={{ padding: '12px' }}>{t('sc_col_version') || 'Version'}</th>
                          <th style={{ padding: '12px' }}>{t('sc_core_hash') || 'Core Hash'}</th>
                          <th style={{ padding: '12px' }}>{t('sc_col_ping') || 'Ping'}</th>
                          <th style={{ padding: '12px' }}>{t('sc_col_action') || 'Aktion'}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(nodeRegistry.registry || []).length === 0 ? (
                          <tr>
                            <td colSpan="6" style={{ padding: '24px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                              {t('sc_no_nodes_in_registry') || 'Noch keine externen Nodes in der Registry.'}
                            </td>
                          </tr>
                        ) : (
                          (nodeRegistry.registry || []).map((entry) => (
                            <tr key={entry.domain} style={{ borderBottom: '1px solid var(--border-color)' }}>
                              <td style={{ padding: '12px' }}>
                                ...
                                <div style={{ color: 'var(--text-secondary)', fontSize: '0.78rem' }}>{entry.operatorGaiaId || (t('sc_operator_not_specified') || 'Operator nicht angegeben')}</div>
                              </td>
                              <td style={{ padding: '12px' }}>
                              ...
                              <td style={{ padding: '12px', maxWidth: '260px', fontFamily: 'monospace', fontSize: '0.75rem', wordBreak: 'break-all' }}>
                                ...
                                {entry.coreHash && entry.coreHash !== nodeRegistry.coreHash && (
                                  <div style={{ color: 'var(--warning)', marginTop: '4px' }}>{t('sc_fork_differs') || 'Update/Fork abweichend'}</div>
                                )}
                              </td>
                              <td style={{ padding: '12px', color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                                {entry.pingCount || 0}x<br />
                                {entry.lastSeenAt ? new Date(entry.lastSeenAt).toLocaleString() : (t('sc_ping_never') || 'nie')}
                              </td>
                              <td style={{ padding: '12px' }}>
                                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                  <button type="button" className="btn-secondary compact-btn" onClick={() => handleRegistryStatus(entry.domain, 'accepted')}>{t('sc_btn_accept') || 'Annehmen'}</button>
                                  <button type="button" className="btn-secondary compact-btn" onClick={() => handleRegistryStatus(entry.domain, 'quarantined')}>{t('sc_btn_quarantine') || 'Quarantaene'}</button>
                                  <button type="button" className="btn-secondary compact-btn" onClick={() => handleRegistryStatus(entry.domain, 'blocked')}>{t('sc_btn_block') || 'Blocken'}</button>
                                </div>
                              </td>
                            </tr>
                          ))
              ...
              {nodeSecrets && (
                <section className="glass-panel" style={{ padding: '20px', borderRadius: '14px', borderLeft: '4px solid var(--accent-cyan)' }}>
                  <h4 style={{ margin: '0 0 10px 0' }}>{t('sc_generated_secrets_title') || 'Generierte Node-Secrets'}</h4>
                  <p style={{ color: 'var(--text-secondary)' }}>
                    {t('sc_generated_secrets_desc') || 'Die Datei wurde serverseitig gespeichert. Trage die Werte als Umgebungsvariablen ein, bevor du den Node produktiv startest.'}
                  </p>
                  <div style={{ display: 'grid', gap: '10px', fontFamily: 'monospace', fontSize: '0.78rem', wordBreak: 'break-all' }}>
                    ...
                    <div><strong>{t('sc_public_key') || 'Public Key'}</strong><br />{nodeSecrets.serverPublicKeyBase64}</div>
                    <div><strong>{t('sc_file') || 'Datei'}</strong><br />{nodeSecrets.savedTo}</div>
                  </div>
                </section>
              )}
            </div>
