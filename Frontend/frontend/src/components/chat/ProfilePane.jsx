...
export const ProfilePane = ({
  ...
  const [accessibilityPrefs, setAccessibilityPrefs] = useState(() => {
    const stored = safeJsonParse(localStorage.getItem('gaiacom_accessibility_preferences'), {});
    return {
      fontScale: ['90', '100', '112', '125'].includes(stored.fontScale) ? stored.fontScale : '100',
  ...
  useEffect(() => {
    ...
    root.style.fontSize = `${accessibilityPrefs.fontScale}%`;
    ...
    try {
      localStorage.setItem('gaiacom_accessibility_preferences', JSON.stringify(accessibilityPrefs));
    } catch (_) {}
  }, [accessibilityPrefs]);

  const updateAccessibilityPreference = patch => {
    setAccessibilityPrefs(current => ({ ...current, ...patch }));
  };
  ...
  const loadDeviceKeys = useCallback(async () => {
    try {
      const result = await api.getDeviceKeys();
      setDeviceKeys(Array.isArray(result?.keys) ? result.keys : []);
    } catch (_) {
      setDeviceKeys([]);
    }
  }, []);
  ...
  useEffect(() => {
    loadDeviceSessions();
    loadDeviceKeys();
  }, [loadDeviceSessions, loadDeviceKeys]);
  ...
  const revokeDeviceKey = async key => {
    if (!key?.id || key.status !== 'active') return;
    try {
      await api.revokeDeviceKey(key.id);
      await loadDeviceKeys();
    } catch (error) {
      setDevicesError(error.message || 'Geraeteschluessel konnte nicht widerrufen werden.');
    }
  };

  const approveNewDevice = async event => {
    event.preventDefault();
    setPairingStatus('');
    const match = /^gaiacom:\/\/pairing\/([0-9a-f-]{36})\/([0-9a-f]{64})$/i.exec(pairingCode.trim());
    if (!match || !activeIdentity?.ID || !derivedKeys?.sign?.private || !mnemonic) {
      setPairingStatus('Der Pairing-Code oder der lokale Schluesselstatus ist ungueltig.');
      return;
    }
    setPairingBusy(true);
    try {
      const [, pairingId, secret] = match;
      const pairing = await api.getDevicePairing(pairingId, secret);
      if (pairing?.identityId !== activeIdentity.ID || pairing?.status !== 'pending') {
        throw new Error('Diese Anfrage gehoert nicht zur aktiven Identitaet oder ist nicht mehr offen.');
      }
      ...
      const signature = crypto.signGsnMessage(transcript, derivedKeys.sign.private);
      const deviceApproval = crypto.signDevicePairingApproval(pairing, encryptedPayload, deviceKeyVault);
      await api.approveDevicePairing(pairingId, secret, encryptedPayload, signature, deviceApproval.approverDeviceKeyId, deviceApproval.deviceSignature);
      setPairingCode('');
      setPairingStatus(`Geraet „${pairing.deviceLabel || 'neu'}“ wurde kryptografisch freigegeben.`);
      await loadDeviceSessions();
    } catch (error) {
      setPairingStatus(error.message || 'Geraetekopplung konnte nicht freigegeben werden.');
    } finally {
      setPairingBusy(false);
    }
  };

  const rotateCurrentDeviceKey = async event => {
    event.preventDefault();
    setRotationStatus('');
    if (!activeIdentity?.ID || !derivedKeys?.sign?.private || !mnemonic || !rotationPassword) {
      setRotationStatus('Dieses Geraet muss mit Passwort entsperrt sein, bevor sein Schluessel rotiert werden kann.');
      return;
    }
    setRotationBusy(true);
    try {
      if (deviceKeyVault?.deviceKeyId) {
        const storedVault = safeJsonParse(localStorage.getItem('gaia_device_key_vault_enc'), null);
        const verifiedVault = await crypto.decryptLocalRecord(storedVault, rotationPassword);
        if (verifiedVault?.deviceKeyId !== deviceKeyVault.deviceKeyId) throw new Error('Das lokale Tresorpasswort stimmt nicht mit der aktiven Geraeteidentitaet ueberein.');
      } else {
        const mnemonicEnvelope = safeJsonParse(localStorage.getItem('gaia_mnemonic_enc'), null);
        const verifiedMnemonic = await crypto.decryptMnemonic(mnemonicEnvelope, rotationPassword);
        if (verifiedMnemonic !== mnemonic) throw new Error('Das lokale Tresorpasswort ist ungueltig.');
      }
      const current = deviceKeys.find(key => key.id === deviceKeyVault?.deviceKeyId);
      const nextKeys = crypto.generateDevicePairingKeys();
      const started = await api.startDevicePairing(
        activeIdentity.ID,
        `${current?.deviceLabel || 'Dieses Geraet'} · Rotation`,
        nextKeys.publicKeys.box,
        nextKeys.publicKeys.pke,
        nextKeys.publicKeys.sign
      );
      ...
      const consumed = await api.consumeDevicePairing(pairing.id, started.secret);
      if (!consumed?.deviceKey?.id) throw new Error('Neuer Geraeteschluessel wurde nicht registriert.');
      const nextVault = {
        version: 1,
        deviceKeyId: consumed.deviceKey.id,
        identityId: activeIdentity.ID,
        privateKeys: nextKeys.privateKeys,
        publicKeys: nextKeys.publicKeys
      };
      const encryptedVault = await crypto.encryptLocalRecord(nextVault, rotationPassword);
      localStorage.setItem('gaia_device_key_vault_enc', JSON.stringify(encryptedVault));
      setDeviceKeyVault(nextVault);
      if (deviceKeyVault?.deviceKeyId) await api.revokeDeviceKey(deviceKeyVault.deviceKeyId);
      setRotationPassword('');
	  setRotationStatus('Rotation erfolgreich: Der vorherige Schluessel ist widerrufen.');
      await loadDeviceKeys();
    } catch (error) {
      setRotationStatus(error.message || 'Geraeteschluessel konnte nicht rotiert werden.');
    } finally {
    ...
    <div className="profile-container" style={{ height: '100%', overflowY: 'auto', padding: '20px' }}>
        ...
        <>
          ...
          <div style={{ maxWidth: '600px', margin: '0 auto', width: '100%' }}>
            <div className="profile-section-card">
              ...
              <p className="profile-section-copy with-bottom-gap">
              ...
              <button type="button" className="btn-action profile-refresh-action" onClick={loadDeviceSessions}>
                {t('refresh') || 'Aktualisieren'}
              </button>
            </div>
            <div className="profile-section-card">
              <div className="profile-section-title">Kryptografische Geräteidentitäten</div>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>Diese Liste ist von Browser-Sitzungen getrennt. Ein Widerruf sperrt die registrierte Geräteidentität dauerhaft für weitere Kopplungszyklen.</p>
              <div className="device-session-list">
                {deviceKeys.length === 0 ? <div className="device-session-card muted">Noch keine kryptografisch gekoppelten Geräte.</div> : deviceKeys.map(key => (
                  <div key={key.id} className={`device-session-card ${key.status === 'active' ? 'current' : 'muted'}`}>
                    <div className="device-session-main"><strong>{key.deviceLabel || 'Gekoppeltes Gerät'}</strong><span>{key.status === 'active' ? 'Aktiv' : 'Widerrufen'}</span><small>Registriert: {formatDeviceTime(key.createdAt)}</small></div>
                    {key.status === 'active' && <button type="button" className="btn-secondary device-revoke-btn" onClick={() => revokeDeviceKey(key)}>Schlüssel widerrufen</button>}
                  </div>
                ))}
              </div>
              <button type="button" className="btn-action profile-refresh-action" onClick={loadDeviceKeys}>Kryptoschlüssel aktualisieren</button>
            </div>
            <form className="profile-section-card" onSubmit={rotateCurrentDeviceKey}>
              <div className="profile-section-title">{deviceKeyVault?.deviceKeyId ? 'Schlüssel dieses Geräts rotieren' : 'Dieses Gerät kryptografisch registrieren'}</div>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>Erzeugt ein neues lokales X25519-, ML-KEM- und Ed25519-Keyset, registriert es atomar und widerruft erst danach die bisherige Geräteidentität.</p>
              <input className="input-field" type="password" value={rotationPassword} onChange={event => setRotationPassword(event.target.value)} placeholder="Lokales Tresorpasswort" autoComplete="current-password" required />
	              {rotationStatus && <p className={rotationStatus.startsWith('Rotation erfolgreich:') ? 'form-success' : 'form-error'}>{rotationStatus}</p>}
              <button type="submit" className="btn-primary" disabled={rotationBusy || !rotationPassword}>{rotationBusy ? 'Schluesselvorgang laeuft...' : (deviceKeyVault?.deviceKeyId ? 'Geraeteschluessel rotieren' : 'Dieses Geraet registrieren')}</button>
            </form>
            <form className="profile-section-card" onSubmit={approveNewDevice}>
              <div className="profile-section-title">Neues Gerät freigeben</div>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                Prüfe den Code direkt auf dem neuen Gerät. Die Freigabe verschlüsselt deine lokale Recovery-Identität ausschließlich für dessen frische Geräteschlüssel.
              </p>
              <textarea className="input-field" value={pairingCode} onChange={event => setPairingCode(event.target.value)} placeholder="gaiacom://pairing/..." autoComplete="off" required />
              {pairingStatus && <p className={pairingStatus.includes('freigegeben') ? 'form-success' : 'form-error'}>{pairingStatus}</p>}
              <button type="submit" className="btn-primary" disabled={pairingBusy || !pairingCode.trim()}>
                {pairingBusy ? 'Freigabe wird signiert...' : 'Geraet kryptografisch freigeben'}
              </button>
            </form>
          </div>
        </>
      ...
      {activeSection === 'accessibility' && (
        <>
          ...
          <div style={{ maxWidth: '760px', margin: '0 auto', width: '100%' }}>
            <div className="profile-section-card accessibility-settings-card">
              <div className="profile-section-title">Darstellung auf diesem Gerät</div>
              <p className="accessibility-copy">
                Diese Optionen gelten lokal für GaiaCom auf diesem Gerät. Sie verändern weder dein Profil noch Nachrichten oder Schlüsselmaterial.
              </p>
              <label className="profile-setting-row">
                <span>
                  <strong>Textgröße</strong>
                  <small>Passt die gesamte Oberflächentypografie an.</small>
                </span>
                <select
                  className="input-field"
                  value={accessibilityPrefs.fontScale}
                  onChange={event => updateAccessibilityPreference({ fontScale: event.target.value })}
                >
                  <option value="90">Kompakt</option>
                  <option value="100">Standard</option>
                  <option value="112">Groß</option>
                  <option value="125">Sehr groß</option>
                </select>
              </label>
              <label className={`privacy-toggle-row ${accessibilityPrefs.highContrast ? 'enabled' : 'disabled'}`}>
                <span>
                  <strong>Hoher Kontrast</strong>
                  <small>Erhöht Kontraste zwischen Text, Steuerflächen und Hintergründen.</small>
                </span>
                <input
                  type="checkbox"
                  checked={accessibilityPrefs.highContrast}
                  onChange={event => updateAccessibilityPreference({ highContrast: event.target.checked })}
                />
              </label>
              <label className={`privacy-toggle-row ${accessibilityPrefs.reduceMotion ? 'enabled' : 'disabled'}`}>
                <span>
                  <strong>Bewegungen reduzieren</strong>
                  <small>Deaktiviert dekorative Animationen und verkürzt Übergänge.</small>
                </span>
                <input
                  type="checkbox"
                  checked={accessibilityPrefs.reduceMotion}
                  onChange={event => updateAccessibilityPreference({ reduceMotion: event.target.checked })}
                />
              </label>
            </div>
          </div>
      ...
      {activeSection === 'notifications' && (
        <>
          ...
          <div style={{ maxWidth: '760px', margin: '0 auto', width: '100%' }}>
            <div className="profile-section-card accessibility-settings-card">
              <div className="profile-section-title">Dieses Gerät</div>
              <p className="accessibility-copy">Diese Einstellungen steuern GaiaCom-Benachrichtigungen für Chat und Mail auf diesem Gerät.</p>
              <label className={`privacy-toggle-row ${notificationPreferences?.enabled !== false ? 'enabled' : 'disabled'}`}>
                <span><strong>Benachrichtigungen aktivieren</strong><small>Neue Chat- und Mail-Eingänge außerhalb des aktiven Gesprächs anzeigen.</small></span>
                <input type="checkbox" checked={notificationPreferences?.enabled !== false} onChange={event => setNotificationPreferences(current => ({ ...current, enabled: event.target.checked }))} />
              </label>
              <label className={`privacy-toggle-row ${notificationPreferences?.showPreview ? 'enabled' : 'disabled'}`}>
                <span><strong>Inhaltsvorschau</strong><small>Zeigt Textauszüge in System-Benachrichtigungen. Standardmäßig verborgen.</small></span>
                <input type="checkbox" checked={notificationPreferences?.showPreview === true} onChange={event => setNotificationPreferences(current => ({ ...current, showPreview: event.target.checked }))} />
              </label>
              <label className={`privacy-toggle-row ${notificationPreferences?.quietHoursEnabled ? 'enabled' : 'disabled'}`}>
                <span><strong>Ruhezeiten</strong><small>Unterdrückt Benachrichtigungen im gewählten Zeitfenster.</small></span>
                <input type="checkbox" checked={notificationPreferences?.quietHoursEnabled === true} onChange={event => setNotificationPreferences(current => ({ ...current, quietHoursEnabled: event.target.checked }))} />
              </label>
              {notificationPreferences?.quietHoursEnabled && <div className="profile-settings-grid"><label className="profile-setting-row"><span><strong>Von</strong></span><input className="input-field" type="time" value={notificationPreferences.quietHoursStart} onChange={event => setNotificationPreferences(current => ({ ...current, quietHoursStart: event.target.value }))} /></label><label className="profile-setting-row"><span><strong>Bis</strong></span><input className="input-field" type="time" value={notificationPreferences.quietHoursEnd} onChange={event => setNotificationPreferences(current => ({ ...current, quietHoursEnd: event.target.value }))} /></label></div>}
            </div>
          </div>
        </>
      )}
