...
export default function AuthScreen({
  ...
  onCancelDevicePairing,
}) {
  const { language, changeLanguage, t } = useTranslation();
  const [activeTab, setActiveTab] = React.useState('auth'); // auth, pair, drop, recovery
  const [pairingLabel, setPairingLabel] = React.useState('');
  ...
  React.useEffect(() => {
    let active = true;
    if (!devicePairing?.pairing?.id || !devicePairing?.secret) {
      setPairingQr('');
      return undefined;
    }
    const payload = `gaiacom://pairing/${devicePairing.pairing.id}/${devicePairing.secret}`;
    QRCode.toDataURL(payload, { errorCorrectionLevel: 'M', margin: 1, width: 280, color: { dark: '#08111f', light: '#f4fbff' } })
      .then(url => { if (active) setPairingQr(url); })
      ...
      <main className="gc-wrap" id="content">
        ...
        <section className="gc-hero" aria-label="GaiaCom Hero">
          <div className="gc-hero-copy">
            ...
            <h1 className="gc-title">
              <span className="gc-gradient">ONE NETWORK.</span>
              <span className="gc-subline">EVERY CONVERSATION.</span>
            </h1>
            <p className="gc-lead gc-network-lead">
              Das quantensichere Kommunikationsnetzwerk für <strong>Chat, E-Mail und soziale Räume</strong> — privat verbunden, global erreichbar und unter deiner Kontrolle.
            </p>
            <p className="gc-sub-lead gc-network-sublead">
              Eine Identität. Ein verschlüsselter Kommunikationsraum. Kein Wechsel zwischen Plattformen, keine Verwertung deiner Beziehungen.
            </p>
            ...
            <div className="gc-entry-points" aria-label="GaiaCom Bereiche">
              <div className="gc-entry-point">
                <span className="gc-entry-point-index">01</span>
                <div>
                  <strong>GaiaChat</strong>
                  <small>Direkte Nachrichten, Gruppen und Räume mit lokalen Schlüsseln.</small>
                </div>
              </div>
              <div className="gc-entry-point">
                <span className="gc-entry-point-index">02</span>
                <div>
                  <strong>GaiaMail</strong>
                  <small>E-Mail im selben sicheren Netzwerk — nicht als Fremdkörper daneben.</small>
                </div>
              </div>
              <div className="gc-entry-point">
                <span className="gc-entry-point-index">03</span>
                <div>
                  <strong>GaiaSpaces</strong>
                  <small>Deine Community, dein Kontext und deine Beziehungen auf einer Ebene.</small>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="gc-visual" aria-label="GaiaCom secure network visualization">
            <div className="gc-floating-card gc-network-status">
              <div className="gc-status-row"><span>GLOBAL COMMUNICATION LAYER</span><strong>LIVE</strong></div>
              <div className="gc-bar"></div>
              <div className="gc-status-row" style={{ marginTop: '12px' }}><span>Quantum-ready encryption</span><strong>ACTIVE</strong></div>
            </div>
            ...
            <div className="gc-terminal gc-network-console" aria-label="GaiaCom Netzwerkübersicht">
              <div className="gc-terminal-top"><span>GAIACOM // COMMUNICATION FABRIC</span><span className="gc-dots"><span></span><span></span><span></span></span></div>
              <div className="gc-network-route"><span className="gc-route-node">CHAT</span><span className="gc-route-line"></span><span className="gc-route-node">IDENTITY</span><span className="gc-route-line"></span><span className="gc-route-node">MAIL</span></div>
              <div className="gc-network-route secondary"><span className="gc-route-node">SPACES</span><span className="gc-route-line"></span><span className="gc-route-node active">YOUR KEYS</span><span className="gc-route-line"></span><span className="gc-route-node">DROP</span></div>
              <p className="gc-network-caption">Ein verschlüsselter Kommunikationslayer — über Formate und Grenzen hinweg.</p>
            </div>
          </div>
        </section>
        ...
        <section className="gc-section">
          <div className="gc-license">
            <article className="gc-license-box"><h3>Client App</h3><strong>GPLv3</strong><p>{t('auth_landing_license1_desc') || 'Maximale Transparenz für Nutzer, Forks und Community-Audits.'}</p></article>
            ...
            <article className="gc-license-box"><h3>Enterprise Modules</h3><strong>Proprietary</strong><p>{t('auth_landing_license3_desc') || 'LDAP, Godmode, Audit, SLA und spezialisierte Business-Logik.'}</p></article>
          </div>
        </section>
      ...
      {showAuthModal && (
        <div className="popup-overlay auth-modal-overlay">
          <div className="auth-card glass-panel auth-modal-card">
            ...
            <div className="auth-boundary-note" role="note">
              <span className="auth-boundary-step">1</span>
              <div>
                <strong>Serverzugang</strong>
                <small>Hier meldest du dich bei deinem GaiaCom-Konto an. Deine lokalen Schlüssel werden dabei nicht an den Server übertragen.</small>
              </div>
              <span className="auth-boundary-next" aria-hidden="true">→</span>
              <span className="auth-boundary-step muted">2</span>
              <div>
                <strong>Lokaler Tresor</strong>
                <small>Nach der Anmeldung entsperrst du deine Schlüssel ausschließlich auf diesem Gerät.</small>
              </div>
            </div>
            ...
            <div className="auth-tabs">
              ...
              {!isRegister && <button
                type="button"
                className={`auth-tab-btn ${activeTab === 'pair' ? 'active' : ''}`}
            ...
            {activeTab === 'auth' ? (
              ...
              <div className="auth-recovery-panel">
                <div className="profile-section-title">Sicheres Gerät koppeln</div>
                <p>Dieses neue Gerät erzeugt eigene X25519- und ML-KEM-Schlüssel. Die Freigabe erfolgt ausschließlich durch ein bereits entsperrtes GaiaCom-Gerät.</p>
                {!devicePairing ? <>
                  <input className="input-field" placeholder={t('auth_username_placeholder') || 'Benutzername'} value={usernameInput} onChange={event => onUsernameChange(event.target.value)} autoComplete="username" />
                  <input className="input-field" type="password" placeholder={t('auth_password_placeholder') || 'Passwort'} value={passwordInput} onChange={event => onPasswordChange(event.target.value)} autoComplete="current-password" />
                  <input className="input-field" placeholder="Name dieses Geräts, z. B. Mein Laptop" value={pairingLabel} onChange={event => setPairingLabel(event.target.value)} maxLength={80} />
                  {devicePairingError && <p className="form-error">{devicePairingError}</p>}
                  <button type="button" className="btn-primary" onClick={() => onStartDevicePairing(pairingLabel)} disabled={!pairingLabel.trim()}>Pairing-Code erzeugen</button>
                </> : <>
                  <p className="auth-copy">Öffne auf deinem bestehenden, entsperrten Gerät unter <strong>Profil → Angemeldete Geräte</strong> die Freigabe und übertrage diesen Code. Er läuft automatisch ab.</p>
                  {pairingQr && <img src={pairingQr} alt="QR-Code zur sicheren Geraetekopplung" className="device-pairing-qr" />}
                  <textarea className="input-field" readOnly value={`gaiacom://pairing/${devicePairing.pairing.id}/${devicePairing.secret}`} aria-label="Pairing-Code" />
                  <button type="button" className="btn-secondary" onClick={() => navigator.clipboard?.writeText(`gaiacom://pairing/${devicePairing.pairing.id}/${devicePairing.secret}`)}>Code kopieren</button>
                  {devicePairingError && <p className="form-error">{devicePairingError}</p>}
                  <button type="button" className="btn-primary" onClick={onCompleteDevicePairing}>Freigabe prüfen und abschließen</button>
                  <button type="button" className="auth-recovery-mode-btn" onClick={onCancelDevicePairing}>Abbrechen</button>
                </>}
              </div>
            ) : activeTab === 'drop' ? (
