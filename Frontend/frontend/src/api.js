...
const DESKTOP_API_ORIGIN = 'https://beta.gaiacom.de';
...
function configuredApiOrigin() {
  ...
  try {
	...
	const allowsInsecureDevEndpoint = import.meta.env.DEV && url.protocol === 'http:';
    if ((url.protocol !== 'https:' && !allowsInsecureDevEndpoint) || url.username || url.password || url.pathname !== '/' || url.search || url.hash) {
      return '';
    }
    return url.origin;
  } catch (_) {
...
try {
  ...
  localStorage.removeItem('gaia_api_url');
} catch (_) {}
...
async function rotateSession() {
  ...
  refreshPromise = (async () => {
    ...
    if (!res.ok) throw new Error('Session refresh rejected');
    const data = await res.json();
    if (isDesktopRuntime()) {
      if (typeof data?.access_token !== 'string' || typeof data?.refresh_token !== 'string') throw new Error('Desktop refresh response incomplete');
...
async function apiFetch(url, options = {}, retry = true) {
  ...
  const authEndpoint = String(url).includes('/api/v1/auth/login') || String(url).includes('/api/v1/auth/register') || String(url).includes('/api/v1/auth/refresh');
  if (res.status === 401 && retry && !authEndpoint) {
    try {
      await rotateSession();
      const headers = { ...(options.headers || {}) };
      if (authToken) headers.Authorization = `Bearer ${authToken}`;
      res = await apiFetch(url, { ...options, headers }, false);
    } catch (_) {
      authToken = '';
      refreshToken = '';
    }
...
export async function login(username, password) {
  ...
  if (isDesktopRuntime()) {
    authToken = typeof data?.access_token === 'string' ? data.access_token : '';
...
export function isTrustedDesktopLocation(location) {
  if (!location) return false;
  return location.protocol === 'tauri:' ||
    location.protocol === 'file:' ||
    (location.protocol === 'http:' && location.hostname === 'tauri.localhost') ||
    (location.protocol === 'https:' && location.hostname === 'tauri.localhost');
}
...
export function isDesktopRuntime() {
  ...
  return !!window.__TAURI__ ||
    ...
    isTrustedDesktopLocation(window.location);
}
...
export async function logout() {
  try {
    const res = await apiFetch(`${BASE_URL}/api/v1/auth/logout`, {
...
export async function getNotificationPreferences() {
  const res = await apiFetch(`${BASE_URL}/api/v1/auth/notification-preferences`, {
    method: 'GET',
    headers: getHeaders()
  });
  return handleResponse(res);
...
export async function saveNotificationPreferences(preferences) {
  const res = await apiFetch(`${BASE_URL}/api/v1/auth/notification-preferences`, {
    ...
    body: JSON.stringify({
      ...
      showPreview: preferences?.showPreview === true,
...
// Device pairing is deliberately separate from login sessions. The pairing secret
// travels only in the QR/manual handover and is never persisted by this client.
export async function startDevicePairing(identityId, deviceLabel, deviceBoxPublic, deviceKemPublic, deviceSignPublic) {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/pairings`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ identityId, deviceLabel, deviceBoxPublic, deviceKemPublic, deviceSignPublic })
  });
  return handleResponse(res);
}

export async function getDevicePairing(pairingId, secret) {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/pairings/${encodeURIComponent(pairingId)}`, {
    method: 'GET',
    headers: getHeaders({ 'X-Gaia-Pairing-Secret': secret })
  });
  return handleResponse(res);
}

export async function approveDevicePairing(pairingId, secret, encryptedPayload, signature, approverDeviceKeyId = '', deviceSignature = '') {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/pairings/${encodeURIComponent(pairingId)}/approve`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ secret, encryptedPayload, signature, approverDeviceKeyId, deviceSignature })
  });
  return handleResponse(res);
}

export async function consumeDevicePairing(pairingId, secret) {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/pairings/${encodeURIComponent(pairingId)}/consume`, {
    method: 'POST',
    headers: getHeaders(),
...
export async function getDeviceKeys() {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/keys`, { method: 'GET', headers: getHeaders() });
  return handleResponse(res);
}

export async function getRecipientDeviceKeys(identityId) {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/recipient-keys?identityId=${encodeURIComponent(identityId)}`, {
    method: 'GET', headers: getHeaders()
  });
  return handleResponse(res);
}

export async function revokeDeviceKey(keyId) {
  const res = await apiFetch(`${BASE_URL}/api/v1/devices/keys/${encodeURIComponent(keyId)}/revoke`, {
    method: 'POST', headers: getHeaders(), body: JSON.stringify({})
  });
  return handleResponse(res);
}
...
export async function createCaseDisclosure(caseId, identityId, role, disclosurePackage, signature = '') {
  const res = await apiFetch(`${BASE_URL}/api/v1/reports/${encodeURIComponent(caseId)}/disclosures`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ identityId, role, package: disclosurePackage, signature })
  });
  return handleResponse(res);
}

export async function requestCaseDisclosure(caseId, identityId, targetRole, reason) {
  const res = await apiFetch(`${BASE_URL}/api/v1/reviewer/cases/${encodeURIComponent(caseId)}/disclosure-requests`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ identityId, targetRole, reason })
  });
  return handleResponse(res);
}

export async function createGaiasEyesGrant(identityId, { caseId = '', scope = 'case', disclosurePackage, signature = '', ttlSeconds = 900 } = {}) {
  const res = await apiFetch(`${BASE_URL}/api/v1/gaias-eyes/grants`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ identityId, caseId, scope, package: disclosurePackage, signature, ttlSeconds })
  });
  return handleResponse(res);
}

export async function consumeGaiasEyesKey(key) {
  const res = await apiFetch(`${BASE_URL}/api/v1/public/gaias-eyes/consume`, {
    method: 'POST',
