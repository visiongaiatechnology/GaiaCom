...
describe('GaiaCom Phase 13 - Platin Cryptographic Verification Harness', () => {
  ...
  test('Device key id is cryptographically bound to the envelope transcript', async () => {
    const deviceKeyId = 'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee';
    const envelope = await crypto.encryptPayload(
      'device-bound payload',
      recipientPubKeysHex,
      senderKeys.keys.sign.private,
    ...
    await expect(crypto.decryptPayload(
      ...
      recipientPubKeysHex,
      recipientPrivKeysHex
    )).resolves.toBe('device-bound payload');

    await expect(crypto.decryptPayload(
      { ...envelope, recipient_device_key_id: 'ffffffff-1111-4222-8333-444444444444' },
      senderKeys.keys.sign.public,
      recipientPubKeysHex,
      recipientPrivKeysHex
    )).rejects.toThrow();
  ...
  test('Top Secret mode requires identity-bound ML-DSA-87 signature', async () => {
    ...
    const tamperedMldsaSignature = (envelope.signature_bundle.ml_dsa_87[0] === '0' ? '1' : '0') + envelope.signature_bundle.ml_dsa_87.slice(1);
    await expect(crypto.decryptPayload(
      {
        ...envelope,
        signature_bundle: {
          ...envelope.signature_bundle,
          ml_dsa_87: tamperedMldsaSignature
        }
      },
      senderKeys.keys.sign.public,
      recipientPubKeysHex,
  ...
  describe('Device Pairing Handover', () => {
    const pairing = {
      id: '8a87fd30-3a85-4f8a-8d12-8b5dc5b4ea11',
      secretHash: 'aa'.repeat(32),
      deviceBoxPublic: '',
      deviceKemPublic: '',
      deviceSignPublic: '',
      expiresAt: '2026-07-11T12:34:56.123456789Z'
    };

    it('encrypts a mnemonic only for the requesting device and binds the exact transcript', async () => {
      const device = crypto.generateDevicePairingKeys();
      pairing.deviceBoxPublic = device.publicKeys.box;
      pairing.deviceKemPublic = device.publicKeys.pke;
      pairing.deviceSignPublic = device.publicKeys.sign;
      const encrypted = await crypto.encryptDevicePairingPayload({ mnemonic: testMnemonic, identityId: 'identity-1' }, pairing);
      const decoded = await crypto.decryptDevicePairingPayload(encrypted, pairing, device.privateKeys);
      expect(decoded.mnemonic).toBe(testMnemonic);
      expect(decoded.identityId).toBe('identity-1');

      const alteredPairing = { ...pairing, expiresAt: '2026-07-11T12:34:56.123456790Z' };
      await expect(crypto.decryptDevicePairingPayload(encrypted, alteredPairing, device.privateKeys)).rejects.toThrow();
    });
  });
});
