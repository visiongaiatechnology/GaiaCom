...
export function useChatNotifications({
  ...
  useEffect(() => {
    ...
    const showNotification = msg => {
      ...
      if (!notificationsAllowed(notificationPreferences)) {
        return;
      }
      ...
      if (isDirectMessage) {
        ...
        new Notification(`Neue Chat-Nachricht von ${senderDisplayName}`, {
          body: notificationBody(notificationPreferences, preview || 'Neue sichere Nachricht', 'Neue sichere Nachricht')
        });
        return;
      }
      ...
      new Notification(`${roomName} · #${channelName}`, {
        body: notificationBody(notificationPreferences, `${senderDisplayName}: ${preview || 'Neue sichere Nachricht'}`, 'Neue sichere Nachricht')
      });
    };

    freshMessages.forEach(showNotification);
  }, [activeChannel, activeChatContact, activeIdentity, activeRoom, channels, chatMessages, contacts, currentMenu, notificationPreferences, rooms, user?.id]); // eslint-disable-line react-hooks/exhaustive-deps
}
