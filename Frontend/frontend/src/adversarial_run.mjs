...
async function run() {
  ...
  const topSecretRecipientPubKeysHex = {
    ...recipientPubKeysHex,
    mldsa87: recipientKeys.keys.mldsa87.public
  };
  const topSecretEnvelope = await crypto.encryptPayload(
    plaintext,
    topSecretRecipientPubKeysHex,
    senderKeys.keys.sign.private,
    "22223333-4444-4555-8666-777788889999",
    Date.now(),
    {
      topSecret: true,
      senderMldsa87PrivHex: senderKeys.keys.mldsa87.private
    }
  );
  const topSecretDecrypted = await crypto.decryptPayload(
    topSecretEnvelope,
    senderKeys.keys.sign.public,
    recipientPubKeysHex,
    recipientPrivKeysHex,
    { expectedSenderMldsa87PubHex: senderKeys.keys.mldsa87.public }
  );
  assert(topSecretDecrypted === plaintext, "Top Secret Ed25519 + ML-DSA-87 message decrypts");

  const missingMldsaSignature = {
    ...topSecretEnvelope,
    signature_bundle: {
      ...topSecretEnvelope.signature_bundle,
      ml_dsa_87: ''
    }
  };
  await assertRejects(
    ...
    "Rejects missing ML-DSA-87 signature in Top Secret mode"
  );

  const wrongMldsaSignature = {
    ...topSecretEnvelope,
    signature_bundle: {
      ...topSecretEnvelope.signature_bundle,
      ml_dsa_87: topSecretEnvelope.signature_bundle.ml_dsa_87.replace(/^../, '00')
    }
  };
  await assertRejects(
    crypto.decryptPayload(
    ...
    "Rejects invalid ML-DSA-87 signature in Top Secret mode"
  );
  ...
  await assertRejects(
    crypto.decryptPayload(
      ...
      recipientPubKeysHex,
      recipientPrivKeysHex,
      { expectedSenderMldsa87PubHex: recipientKeys.keys.mldsa87.public }
    ),
    "Rejects ML-DSA-87 public key mismatch against sender identity"
  );
  ...
  assert(!/console\.(log|warn|error)\([^)]*(mnemonic|private|secret|seed)/i.test(joinedSources), "No console logging of secret-bearing values");
	...
	const appMainContentSource = fs.readFileSync(path.join(__dirname, 'components', 'app', 'AppMainContent.jsx'), 'utf8');
	const authScreenSource = fs.readFileSync(path.join(__dirname, 'components', 'auth', 'AuthScreen.jsx'), 'utf8');
	const securityCenterSource = fs.readFileSync(path.join(__dirname, 'components', 'chat', 'SecurityCenter.jsx'), 'utf8');
	const chatPaneSource = fs.readFileSync(path.join(__dirname, 'components', 'chat', 'ChatPane.jsx'), 'utf8');
	const publicChannelsPaneSource = fs.readFileSync(path.join(__dirname, 'components', 'chat', 'PublicChannelsPane.jsx'), 'utf8');
  const apiSource = fs.readFileSync(path.join(__dirname, 'api.js'), 'utf8');
	const listPaneSource = fs.readFileSync(path.join(__dirname, 'components', 'layout', 'ListPane.jsx'), 'utf8');
	const groupPaneSource = fs.readFileSync(path.join(__dirname, 'components', 'chat', 'GroupChatPane.jsx'), 'utf8');
	const dropPaneSource = fs.readFileSync(path.join(__dirname, 'components', 'chat', 'DropPane.jsx'), 'utf8');
  const useEmailsSource = fs.readFileSync(path.join(__dirname, 'hooks', 'useEmails.js'), 'utf8');
  const cssSource = fs.readFileSync(path.join(__dirname, 'index.css'), 'utf8');
	const i18nSource = fs.readFileSync(path.join(__dirname, 'utils', 'i18n.jsx'), 'utf8');
  const usePresenceSource = fs.readFileSync(path.join(__dirname, 'hooks', 'usePresence.js'), 'utf8');
  const deLocaleSource = fs.readFileSync(path.join(__dirname, 'utils', 'locales', 'de.js'), 'utf8');
  ...
  assert(
    appMainContentSource.includes('className="mobile-floating-menu mobile-menu-toggle"') &&
  ...
  assert(
    /\.auth-product-shell[\s\S]*?height:\s*100dvh !important[\s\S]*?overflow-y:\s*auto !important[\s\S]*?scrollbar-gutter:\s*stable/.test(joinedSources),
    "Landing page exposes a stable viewport scrollbar"
  );
  assert(
    authScreenSource.includes("gaia_cookie_ack") &&
      authScreenSource.includes("gaia-cookie-banner") &&
      authScreenSource.includes("keine Drittanbieter-Profile"),
    "Sovereign cookie banner is present without third-party tracking claims"
  );
  assert(
    authScreenSource.includes("legalAccepted") &&
      authScreenSource.includes("auth-consent-check") &&
      authScreenSource.includes("Datenschutzbestimmungen und Nutzungsbedingungen") &&
      authScreenSource.includes("disabled={isRegister && !legalAccepted}"),
    "Registration requires privacy and terms consent before submit"
  );
  assert(
    securityCenterSource.includes("operatorAccess") &&
      securityCenterSource.includes("api.getNodeSecuritySummary()") &&
      securityCenterSource.includes("roles.includes('node_operator') || operatorAccess") &&
      securityCenterSource.includes("Node-Sicherheitsdaten nicht verfügbar"),
    "Security Center grants node operator view from backend-confirmed node access"
  );
  assert(
    useEmailsSource.includes("api.getMailboxMessages(activeIdentity.ID, { folder: 'all', limit: 200 })"),
    "Mailbox polling includes sent self-copies so sent chats stay visible"
  );
  assert(
    apiSource.includes('togglePublicChannelPostReaction') &&
      apiSource.includes('/api/v1/public-channels/posts/reaction') &&
      apiSource.includes('createPublicChannelPostComment') &&
      apiSource.includes('/api/v1/public-channels/posts/comment') &&
      apiSource.includes('updatePublicChannelPostPin') &&
      apiSource.includes('/api/v1/public-channels/posts/pin') &&
      apiSource.includes('updatePublicChannelComments') &&
      apiSource.includes('/api/v1/public-channels/comments'),
    "Public channel post reactions and comments use backend APIs"
  );
  assert(
    publicChannelsPaneSource.includes('post.reactionState?.reactions') &&
      publicChannelsPaneSource.includes('post.reactionState?.reactedByMe') &&
      publicChannelsPaneSource.includes('Array.isArray(post.comments)') &&
      publicChannelsPaneSource.includes('post.isPinned') &&
      publicChannelsPaneSource.includes('activePublicChannel.commentsEnabled !== false') &&
      !publicChannelsPaneSource.includes('setPostReactions') &&
      !publicChannelsPaneSource.includes('setPostComments') &&
      !publicChannelsPaneSource.includes('pinnedPostIds'),
    "Public channel post reactions/comments/pins are server-backed, not local-only"
  );
  assert(
    !chatPaneSource.includes('isPeerTyping') &&
      !chatPaneSource.includes('setIsPeerTyping') &&
      !chatPaneSource.includes('Simulated peer typing') &&
      apiSource.includes('/api/v1/presence/typing') &&
      apiSource.includes('getTypingStatus') &&
      apiSource.includes('updateTypingStatus') &&
      chatPaneSource.includes('tippt gerade...') &&
      groupPaneSource.includes('tippt gerade...'),
    "Chat typing indicators are server-backed instead of fake local peer typing"
  );
  assert(
    chatPaneSource.includes("msg.isRead") &&
      chatPaneSource.includes("msg.delivered") &&
      chatPaneSource.includes("label: 'Gelesen'") &&
      chatPaneSource.includes("label: 'Zugestellt'") &&
      chatPaneSource.includes("label: 'Gesendet'") &&
      useEmailsSource.includes('delivered: deliveredState') &&
      useEmailsSource.includes('readReceiptSourceId:'),
    "Chat delivery/read status distinguishes sent, delivered, and read"
  );
  assert(
    apiSource.includes('sendPresenceHeartbeat') &&
      apiSource.includes('/api/v1/presence/heartbeat') &&
      apiSource.includes('getPresenceStatus') &&
      apiSource.includes('/api/v1/presence/status') &&
      usePresenceSource.includes('api.sendPresenceHeartbeat(activeIdentity.ID') &&
      usePresenceSource.includes('api.getPresenceStatus(gaiaIds)') &&
      chatPaneSource.includes('contactPresence') &&
      chatPaneSource.includes('chat-presence-status'),
    "Chat header uses backend presence heartbeat and status polling"
  );
  assert(
    useEmailsSource.includes('untrusted: !!(env.Untrusted || env.untrusted || isLegacySmtp'),
    "Legacy SMTP is always marked untrusted in mail models"
  );
  assert(
    listPaneSource.includes("mail.isSmtp ? (") &&
      listPaneSource.includes("⚠️ Legacy SMTP") &&
      (deLocaleSource.includes('smtp_legacy_badge: "Legacy SMTP / Unsicher"') ||
       deLocaleSource.includes('"smtp_legacy_badge": "Legacy SMTP / Unsicher"')),
