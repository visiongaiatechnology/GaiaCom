...
import { DEFAULT_NOTIFICATION_PREFERENCES, normalizeNotificationPreferences } from './utils/notificationPreferences';
...
export default function App() {
  ...
  const [notificationPreferences, setNotificationPreferences] = useState(() => normalizeNotificationPreferences(
    safeStorageJson(localStorage, 'gaiacom_notification_preferences', DEFAULT_NOTIFICATION_PREFERENCES)
  ));
  const notificationPreferencesHydratedForUserRef = useRef('');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('gaiacom_notification_preferences', JSON.stringify(notificationPreferences));
    } catch (_) {}
  ...
  const [currentMenu, setCurrentMenuState] = useState('dashboard'); // 'dashboard', 'inbox', 'smtp_inbox', 'sent', 'groups', 'chat', 'contacts', 'profile'
  const setCurrentMenu = useCallback((nextMenu) => {
    startTransition(() => setCurrentMenuState(nextMenu));
  }, []);
  ...
  useEffect(() => {
    let active = true;
    notificationPreferencesHydratedForUserRef.current = '';
    if (!user?.id) return undefined;

    api.getNotificationPreferences()
      .then(preferences => {
        if (active) setNotificationPreferences(normalizeNotificationPreferences(preferences));
      })
      .catch(() => {})
      .finally(() => {
        if (active) notificationPreferencesHydratedForUserRef.current = user.id;
      });
    return () => { active = false; };
  }, [user?.id]);

  useEffect(() => {
    if (!user?.id || notificationPreferencesHydratedForUserRef.current !== user.id) return;
    api.saveNotificationPreferences(notificationPreferences).catch(() => {});
  ...
  if (!user || !derivedKeys) {
    return (
      <AuthScreen
        ...
        onStartDevicePairing={startDevicePairing}
        onCompleteDevicePairing={completeDevicePairing}
        onCancelDevicePairing={cancelDevicePairing}
    ...
    <>
      {showBootSequence && (
        <div className="gaia-boot-overlay" aria-hidden="true">
          ...
          <span className="gaia-boot-logo-shell">
            <img className="gaia-boot-logo" src={gaiacomLogo} alt="GaiaCom Logo" />
          </span>
      ...
      {showTransition && (
        <div className="gaia-transition-overlay" aria-hidden="true">
          <div className="gaia-transition-logo-container">
            <div className="gaia-transition-ring-wrapper">
              <div className="gaia-transition-ring"></div>
              <img className="gaia-transition-logo" src={gaiacomLogo} alt="GaiaCom Logo" />
            </div>
      ...
      <AppMainContent
        ...
        setDeviceKeyVault={setDeviceKeyVault}
