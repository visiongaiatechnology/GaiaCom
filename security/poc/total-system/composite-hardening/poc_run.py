...
def assert_no_match(label, pattern, roots):
    ...
    for root in roots:
        for path in root.rglob("*"):
            ...
            if {"node_modules", "build", "dist", "target", ".git", ".cache"} & parts:
                continue
            if any(part.startswith((".gocache", ".gomodcache")) for part in path.parts):
                continue
...
def assert_json_parse_allowlist(label, root):
    ...
    for path in root.rglob("*"):
        if path.suffix.lower() not in {".js", ".jsx"}:
            continue
        if not path.is_file() or {"node_modules", "build", "dist", "target"} & set(path.parts):
            continue
        if any(part.startswith((".gocache", ".gomodcache")) for part in path.parts):
            continue
