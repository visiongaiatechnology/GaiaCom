...
func (s *SQLStore) CreateFileChunk(chunk *models.FileChunk) error {
	result, err := s.execWithBusyRetry(
		context.Background(),
		`INSERT INTO file_chunks (file_id, chunk_index, chunk_hash, chunk_size, minio_id)
		 SELECT ?, ?, ?, ?, ?
		 FROM file_metadata
	...
	result, err := s.execWithBusyRetry(
		...
		`UPDATE file_metadata
		 ...
		 WHERE file_id = ?
		   AND user_id = ?
		   AND status = 'pending'
		   AND EXISTS (SELECT 1 FROM file_chunks WHERE file_id = file_metadata.file_id)
		   AND file_size = (SELECT COALESCE(SUM(chunk_size), -1) FROM file_chunks WHERE file_id = file_metadata.file_id)
		   AND 0 = (SELECT MIN(chunk_index) FROM file_chunks WHERE file_id = file_metadata.file_id)
		   AND (SELECT MAX(chunk_index) FROM file_chunks WHERE file_id = file_metadata.file_id) =
		       (SELECT COUNT(*) - 1 FROM file_chunks WHERE file_id = file_metadata.file_id)
		   AND NOT EXISTS (
		       SELECT 1 FROM file_chunks
		       WHERE file_id = file_metadata.file_id AND chunk_size <= 0
		   )`,
		formatTime(utcNow()),
		fileID,
		userID,
