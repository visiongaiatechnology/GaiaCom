...
const federationQueueLeaseDuration = 2 * time.Minute
...
func (s *SQLStore) AddFederationQueueItem(item *models.FederationQueue) error {
	...
	result, err := s.db.ExecContext(
		context.Background(),
		`INSERT INTO federation_queues (pdu_id, pdu_payload, target_url, status, attempts, last_error, next_retry, lease_owner, lease_until, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, '', '', ?, ?)`,
		item.PDUID,
...
func (s *SQLStore) ClaimNextFederationQueueItem(ctx context.Context) (*models.FederationQueue, error) {
	...
	item, err := scanFederationQueue(tx.QueryRowContext(
		ctx,
		`SELECT id, pdu_id, pdu_payload, target_url, status, attempts, last_error, next_retry, lease_owner, lease_until, created_at, updated_at
		 FROM federation_queues
		 WHERE (status = ? AND next_retry <= ?)
		    OR (status = ? AND lease_until <= ?)
		 ORDER BY CASE WHEN status = ? THEN 0 ELSE 1 END, next_retry ASC
		 LIMIT 1`,
		string(models.QueueStatusPending),
		formatTime(now),
		string(models.QueueStatusSending),
		formatTime(now),
		string(models.QueueStatusSending),
	))
	...
	item.Attempts++
	item.UpdatedAt = now
	item.LeaseOwner = uuid.New().String()
	item.LeaseUntil = now.Add(federationQueueLeaseDuration)
	result, err := tx.ExecContext(
		ctx,
		`UPDATE federation_queues
		 SET status = ?, attempts = ?, lease_owner = ?, lease_until = ?, updated_at = ?
		 WHERE id = ?
		   AND ((status = ? AND next_retry <= ?) OR (status = ? AND lease_until <= ?))`,
		string(item.Status),
		item.Attempts,
		item.LeaseOwner,
		formatTime(item.LeaseUntil),
		formatTime(item.UpdatedAt),
		item.ID,
		string(models.QueueStatusPending),
		formatTime(now),
...
func (s *SQLStore) DeleteFederationQueueItem(ctx context.Context, item *models.FederationQueue) error {
	if item == nil || item.ID == 0 || item.LeaseOwner == "" {
		return errors.New("invalid federation queue lease")
	}
	result, err := s.db.ExecContext(ctx, `DELETE FROM federation_queues WHERE id = ? AND status = ? AND lease_owner = ?`, item.ID, string(models.QueueStatusSending), item.LeaseOwner)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
	...
	if affected != 1 {
		return errors.New("federation queue lease lost")
	}
	return nil
}
...
func (s *SQLStore) SaveFederationQueueItem(ctx context.Context, item *models.FederationQueue) error {
	...
	result, err := s.db.ExecContext(
		ctx,
		`UPDATE federation_queues
		 SET pdu_id = ?, pdu_payload = ?, target_url = ?, status = ?, attempts = ?, last_error = ?, next_retry = ?, lease_owner = '', lease_until = '', updated_at = ?
	...
	if affected != 1 {
		return fmt.Errorf("federation queue lease lost for item %d", item.ID)
	}
	item.LeaseOwner = ""
