...
type DistributedRateLimitStore interface {
	IncrementSecurityRateLimit(ctx context.Context, key string, window time.Duration) (int, error)
	GetSecurityRateLimitCount(ctx context.Context, key string) (int, error)
	ResetSecurityRateLimit(ctx context.Context, key string) error
}
...
type AuthStore interface {
	...
	UpdateUserPasswordAndRevokeSessions(ctx context.Context, userID uuid.UUID, passwordHash string, keepSessionID uuid.UUID) error
	...
	SaveNotificationPreferences(ctx context.Context, preferences *models.NotificationPreferences) error
	CreateDevicePairing(ctx context.Context, pairing *models.DevicePairing) error
	FindDevicePairing(ctx context.Context, pairingID uuid.UUID) (*models.DevicePairing, error)
	ApproveDevicePairing(ctx context.Context, pairingID uuid.UUID, encryptedPayload, signature string, approvedAt time.Time) error
	ConsumeDevicePairing(ctx context.Context, pairingID, sessionID uuid.UUID, consumedAt time.Time) (*models.DeviceKey, error)
	FindDeviceKeysForUser(ctx context.Context, userID uuid.UUID) ([]models.DeviceKey, error)
	FindActiveDeviceKeysForIdentity(ctx context.Context, identityID uuid.UUID) ([]models.DeviceKey, error)
	FindActiveDeviceKey(ctx context.Context, identityID, keyID uuid.UUID) (*models.DeviceKey, error)
	RevokeDeviceKey(ctx context.Context, userID, keyID uuid.UUID, revokedAt time.Time) error
	...
	SetDeviceSessionRefresh(ctx context.Context, sessionID uuid.UUID, tokenHash string, familyID uuid.UUID, expiresAt time.Time) error
	FindDeviceSessionForRefresh(ctx context.Context, sessionID uuid.UUID) (*models.DeviceSession, error)
	RotateDeviceSessionRefresh(ctx context.Context, sessionID uuid.UUID, expectedHash, nextHash string, nextExpiresAt time.Time) (bool, error)
	RevokeDeviceSession(ctx context.Context, userID uuid.UUID, sessionID uuid.UUID) error
	RevokeAllDeviceSessionsExcept(ctx context.Context, userID uuid.UUID, keepSessionID uuid.UUID) error
}
...
type FederationStore interface {
	...
	DeleteFederationQueueItem(ctx context.Context, item *models.FederationQueue) error
...
type FederationReplayStore interface {
	ClaimFederationPDU(ctx context.Context, origin string, pduID string, leaseUntil time.Time) (bool, error)
	CompleteFederationPDU(ctx context.Context, origin string, pduID string, processedAt time.Time) error
	ReleaseFederationPDU(ctx context.Context, origin string, pduID string) error
}
...
type GovernanceStore interface {
	...
	CreateAbuseDisclosurePackage(ctx context.Context, pkg *models.AbuseDisclosurePackage) error
	GetAbuseDisclosurePackages(ctx context.Context, caseID string) ([]models.AbuseDisclosurePackage, error)
	CreateAbuseDisclosureRequest(ctx context.Context, req *models.AbuseDisclosureRequest) error
	GetAbuseDisclosureRequests(ctx context.Context, caseID string) ([]models.AbuseDisclosureRequest, error)

	CreateGaiasEyesGrant(ctx context.Context, grant *models.GaiasEyesGrant) error
	GetGaiasEyesGrantByTokenHash(ctx context.Context, tokenHash string) (*models.GaiasEyesGrant, error)
	MarkGaiasEyesGrantUsed(ctx context.Context, id string, usedAt time.Time) error
