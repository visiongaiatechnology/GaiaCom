...
func (s *SQLStore) CreateAbuseDisclosurePackage(ctx context.Context, pkg *models.AbuseDisclosurePackage) error {
	_, err := s.execWithBusyRetry(ctx,
		`INSERT INTO abuse_disclosure_packages (id, case_id, owner_identity, disclosure_role, package_hash, package_data, signature, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		pkg.ID, pkg.CaseID, pkg.OwnerIdentity, pkg.DisclosureRole, pkg.PackageHash, string(pkg.PackageData), pkg.Signature, formatTime(pkg.CreatedAt),
	)
	return err
}

func (s *SQLStore) GetAbuseDisclosurePackages(ctx context.Context, caseID string) ([]models.AbuseDisclosurePackage, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, case_id, owner_identity, disclosure_role, package_hash, package_data, signature, created_at FROM abuse_disclosure_packages WHERE case_id = ? ORDER BY created_at ASC`, caseID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var list []models.AbuseDisclosurePackage
	for rows.Next() {
		var pkg models.AbuseDisclosurePackage
		var packageData []byte
		var createdAt string
		if err := rows.Scan(&pkg.ID, &pkg.CaseID, &pkg.OwnerIdentity, &pkg.DisclosureRole, &pkg.PackageHash, &packageData, &pkg.Signature, &createdAt); err != nil {
			return nil, err
		}
		pkg.PackageData = models.JSONB(packageData)
...
func (s *SQLStore) CreateAbuseDisclosureRequest(ctx context.Context, req *models.AbuseDisclosureRequest) error {
	_, err := s.execWithBusyRetry(ctx,
		`INSERT INTO abuse_disclosure_requests (id, case_id, requester_identity, target_role, reason, status, created_at, responded_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		req.ID, req.CaseID, req.RequesterIdentity, req.TargetRole, req.Reason, req.Status, formatTime(req.CreatedAt), req.RespondedAt,
	)
	return err
}

func (s *SQLStore) GetAbuseDisclosureRequests(ctx context.Context, caseID string) ([]models.AbuseDisclosureRequest, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, case_id, requester_identity, target_role, reason, status, created_at, responded_at FROM abuse_disclosure_requests WHERE case_id = ? ORDER BY created_at ASC`, caseID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var list []models.AbuseDisclosureRequest
	for rows.Next() {
		var req models.AbuseDisclosureRequest
		var createdAt string
		if err := rows.Scan(&req.ID, &req.CaseID, &req.RequesterIdentity, &req.TargetRole, &req.Reason, &req.Status, &createdAt, &req.RespondedAt); err != nil {
			return nil, err
		}
...
func (s *SQLStore) CreateGaiasEyesGrant(ctx context.Context, grant *models.GaiasEyesGrant) error {
	_, err := s.execWithBusyRetry(ctx,
		`INSERT INTO gaias_eyes_grants (id, token_hash, owner_identity, case_id, scope, package_hash, package_data, signature, status, expires_at, used_at, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		grant.ID, grant.TokenHash, grant.OwnerIdentity, grant.CaseID, grant.Scope, grant.PackageHash, string(grant.PackageData), grant.Signature, grant.Status, formatTime(grant.ExpiresAt), grant.UsedAt, formatTime(grant.CreatedAt),
	)
	return err
}

func (s *SQLStore) GetGaiasEyesGrantByTokenHash(ctx context.Context, tokenHash string) (*models.GaiasEyesGrant, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, token_hash, owner_identity, case_id, scope, package_hash, package_data, signature, status, expires_at, used_at, created_at FROM gaias_eyes_grants WHERE token_hash = ? LIMIT 1`, tokenHash)
	var grant models.GaiasEyesGrant
	var packageData []byte
	var expiresAt, createdAt string
	if err := row.Scan(&grant.ID, &grant.TokenHash, &grant.OwnerIdentity, &grant.CaseID, &grant.Scope, &grant.PackageHash, &packageData, &grant.Signature, &grant.Status, &expiresAt, &grant.UsedAt, &createdAt); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, err
	}
	grant.PackageData = models.JSONB(packageData)
	grant.ExpiresAt = parseTime(expiresAt)
	grant.CreatedAt = parseTime(createdAt)
	return &grant, nil
}

func (s *SQLStore) MarkGaiasEyesGrantUsed(ctx context.Context, id string, usedAt time.Time) error {
	result, err := s.execWithBusyRetry(ctx, `UPDATE gaias_eyes_grants SET status = 'used', used_at = ? WHERE id = ? AND status = 'active'`, formatTime(usedAt), id)
	if err != nil {
		return err
	}
