...
func (s *MessagingService) SaveAndDistributeMessage(ctx context.Context, userID uuid.UUID, senderID uuid.UUID, envelopeData []byte, recipientIDs []uuid.UUID) (uuid.UUID, error) {
	...
	{
		deviceStore, ok := s.Identities.(interface {
			FindActiveDeviceKeysForIdentity(context.Context, uuid.UUID) ([]models.DeviceKey, error)
			FindActiveDeviceKey(context.Context, uuid.UUID, uuid.UUID) (*models.DeviceKey, error)
		})
		if !ok {
			return uuid.Nil, errors.New("device key store unavailable")
		}
		senderKeys, err := deviceStore.FindActiveDeviceKeysForIdentity(ctx, senderID)
		if err != nil {
			return uuid.Nil, errors.New("sender device keys unavailable")
		}
		if len(senderKeys) > 0 {
			senderKeyID, err := uuid.Parse(meta.SenderDeviceKeyID)
			if err != nil || senderKeyID == uuid.Nil {
				return uuid.Nil, errors.New("sender device proof required")
			}
			senderKey, err := deviceStore.FindActiveDeviceKey(ctx, senderID, senderKeyID)
			if err != nil {
				return uuid.Nil, errors.New("sender device key revoked or unknown")
			}
			publicKey, pubErr := hex.DecodeString(senderKey.SignPublic)
			signature, sigErr := hex.DecodeString(meta.DeviceSignature)
			proof := strings.Join([]string{"gaiacom-device-envelope-v1", senderKeyID.String(), meta.ClientMessageID, meta.EnvelopeSignature}, "\n")
			if pubErr != nil || sigErr != nil || len(publicKey) != ed25519.PublicKeySize || len(signature) != ed25519.SignatureSize || !ed25519.Verify(ed25519.PublicKey(publicKey), []byte(proof), signature) {
				return uuid.Nil, errors.New("sender device proof rejected")
			}
		} else if meta.SenderDeviceKeyID != "" || meta.DeviceSignature != "" {
			return uuid.Nil, errors.New("sender device proof unavailable")
		}
		if len(recipients) != 1 {
			return uuid.Nil, errors.New("direct device envelope requires one recipient")
		}
		activeKeys, err := deviceStore.FindActiveDeviceKeysForIdentity(ctx, recipients[0])
		if err != nil {
			return uuid.Nil, errors.New("recipient device keys unavailable")
		}
		if len(activeKeys) > 0 {
			keyID, err := uuid.Parse(meta.RecipientDeviceKeyID)
			if err != nil || keyID == uuid.Nil {
				return uuid.Nil, errors.New("recipient device key required")
			}
			key, err := deviceStore.FindActiveDeviceKey(ctx, recipients[0], keyID)
			if err != nil {
				return uuid.Nil, errors.New("recipient device key revoked or unknown")
			}
			if !strings.EqualFold(strings.TrimSpace(meta.RecipientDeviceBoxPublic), key.BoxPublic) {
				return uuid.Nil, errors.New("recipient device key binding rejected")
			}
		} else if meta.RecipientDeviceKeyID != "" {
			return uuid.Nil, errors.New("recipient device key unavailable")
		}
	}
...
func (s *MessagingService) EditDirectMessageForUser(ctx context.Context, userID uuid.UUID, identityID uuid.UUID, originalMessageID uuid.UUID, peerEnvelopeData []byte, selfEnvelopeData []byte) (uuid.UUID, error) {
	...
	activeKeys, err := deviceStore.FindActiveDeviceKeysForIdentity(ctx, identityID)
	if err != nil {
		return uuid.Nil, errors.New("sender device keys unavailable")
	}
	if len(activeKeys) > 0 {
		return uuid.Nil, errors.New("device fanout revisions require a new message")
	}
