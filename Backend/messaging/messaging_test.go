...
func TestDeviceEnvelopeRequiresActiveSenderAndRecipientKeys(t *testing.T) {
	store, cleanup := setupTestDBAndStore(t)
	defer cleanup()
	ctx := context.Background()
	create := func(name string) (*models.User, *models.Identity, ed25519.PrivateKey) {
		user := &models.User{ID: uuid.New(), Username: name, PasswordHash: "hash", PublicKey: "pk"}
		if err := store.CreateUser(user); err != nil {
			t.Fatal(err)
		}
		pub, priv, _ := ed25519.GenerateKey(rand.Reader)
		identity := &models.Identity{ID: uuid.New(), UserID: user.ID, GaiaID: "@" + name + ":gaiacom.local", PublicRecord: models.JSONB(`{"public_keys":{"identity":"` + hex.EncodeToString(pub) + `"}}`), IsActive: true}
		if err := store.CreateIdentity(identity); err != nil {
			t.Fatal(err)
		}
		return user, identity, priv
	}
	pair := func(user *models.User, identity *models.Identity, identityPriv ed25519.PrivateKey) (*models.DeviceKey, ed25519.PrivateKey) {
		box, kem := make([]byte, 32), make([]byte, 1568)
		_, _ = rand.Read(box)
		_, _ = rand.Read(kem)
		devicePub, devicePriv, _ := ed25519.GenerateKey(rand.Reader)
		svc := devices.NewService(store)
		pairing, secret, err := svc.Start(ctx, user.ID, identity.ID, "device", hex.EncodeToString(box), hex.EncodeToString(kem), hex.EncodeToString(devicePub))
		if err != nil {
			t.Fatal(err)
		}
		payload := "encrypted-handover-payload-long-enough-for-validation"
		approval := ed25519.Sign(identityPriv, []byte(devices.ApprovalPayload(pairing, payload)))
		if err := svc.Approve(ctx, user.ID, pairing.ID, secret, payload, hex.EncodeToString(approval), "", ""); err != nil {
			t.Fatal(err)
		}
		key, err := svc.Consume(ctx, user.ID, pairing.ID, uuid.Nil, secret)
		if err != nil {
			t.Fatal(err)
		}
		return key, devicePriv
	}
	senderUser, senderIdentity, senderIdentityPriv := create("device-sender")
	recipientUser, recipientIdentity, recipientIdentityPriv := create("device-recipient")
	senderKey, senderDevicePriv := pair(senderUser, senderIdentity, senderIdentityPriv)
	recipientKey, _ := pair(recipientUser, recipientIdentity, recipientIdentityPriv)
	makeEnvelope := func() []byte {
		messageID, envelopeSignature := uuid.New().String(), "identity-signature"
		proof := "gaiacom-device-envelope-v1\n" + senderKey.ID.String() + "\n" + messageID + "\n" + envelopeSignature
		deviceSignature := ed25519.Sign(senderDevicePriv, []byte(proof))
		return []byte(`{"client_message_id":"` + messageID + `","signature":"` + envelopeSignature + `","sender_device_key_id":"` + senderKey.ID.String() + `","device_signature":"` + hex.EncodeToString(deviceSignature) + `","recipient_device_key_id":"` + recipientKey.ID.String() + `","recipient_device_box_public":"` + recipientKey.BoxPublic + `"}`)
	}
	svc := NewMessagingService(store, store)
	if _, err := svc.SaveAndDistributeMessage(ctx, senderUser.ID, senderIdentity.ID, makeEnvelope(), []uuid.UUID{recipientIdentity.ID}); err != nil {
		t.Fatalf("active device envelope rejected: %v", err)
	}
	if _, err := svc.EditDirectMessageForUser(ctx, senderUser.ID, senderIdentity.ID, uuid.New(), []byte(`{"payload":"peer"}`), []byte(`{"payload":"self"}`)); err == nil {
		t.Fatal("legacy edit path accepted for a device-secured identity")
	}
	if err := store.RevokeDeviceKey(ctx, recipientUser.ID, recipientKey.ID, time.Now().UTC()); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.SaveAndDistributeMessage(ctx, senderUser.ID, senderIdentity.ID, makeEnvelope(), []uuid.UUID{recipientIdentity.ID}); err == nil {
		t.Fatal("revoked recipient device key accepted")
	}
}
