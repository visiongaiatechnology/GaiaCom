...
func setupTestStore(t *testing.T) (*repository.SQLStore, func()) {
	...
	t.Setenv("GAIACOM_SERVER_NAME", "node.test.example")
...
func setupTestHandler(t *testing.T, store repository.Store) http.Handler {
	t.Helper()
	handler, err := SetupRoutes(store)
	if err != nil {
		t.Fatalf("initialize test routes: %v", err)
	}
	return handler
}
...
func TestAdversarialCSPHeaders(t *testing.T) {
	...
	handlerProd := setupTestHandler(t, store)
	...
	handlerDev := setupTestHandler(t, store)
...
func TestPairingCORSPreflightAllowsSecretHeader(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()
	handler := setupTestHandler(t, store)
	req := httptest.NewRequest(http.MethodOptions, "/api/v1/devices/pairings/00000000-0000-0000-0000-000000000001", nil)
	req.Header.Set("Origin", "tauri://localhost")
	req.Header.Set("Access-Control-Request-Method", http.MethodGet)
	req.Header.Set("Access-Control-Request-Headers", "X-Gaia-Pairing-Secret")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusNoContent {
		t.Fatalf("pairing preflight rejected: %d", rec.Code)
	}
	if !strings.Contains(rec.Header().Get("Access-Control-Allow-Headers"), "X-Gaia-Pairing-Secret") {
		t.Fatal("pairing secret header missing from CORS allowlist")
	}
}
