...
// NotificationPreferences are account defaults. Delivery remains device-local,
// while privacy-sensitive preview and quiet-hour choices stay consistent across
// a user's signed-in clients.
...
type DeviceSession struct {
	...
	UserID           uuid.UUID `json:"userId"`
	DeviceKeyID      uuid.UUID `json:"deviceKeyId,omitempty"`
	...
	RefreshFamilyID  uuid.UUID `json:"-"`
	RefreshExpiresAt time.Time `json:"-"`
}

// DevicePairing stores only public device keys and an end-to-end encrypted
// handover payload. Private keys and mnemonic material never reach the server.
type DevicePairing struct {
	ID         uuid.UUID `json:"id"`
	UserID     uuid.UUID `json:"userId"`
	IdentityID uuid.UUID `json:"identityId"`
	// SecretHash is a public commitment to the 256-bit pairing secret. It is
	// required in the client-side approval transcript; the secret itself is
	// never returned by a read endpoint or stored outside the initiating device.
...
// DeviceKey is a long-lived public device identity. It is deliberately
// separate from an HTTP session: revoking a browser session does not erase the
// cryptographic audit trail, while key revocation prevents future device use.
type DeviceKey struct {
	ID          uuid.UUID `json:"id"`
	UserID      uuid.UUID `json:"userId"`
	IdentityID  uuid.UUID `json:"identityId"`
	DeviceLabel string    `json:"deviceLabel"`
	BoxPublic   string    `json:"boxPublic"`
...
type AbuseDisclosurePackage struct {
	ID             string    `json:"id"`
	CaseID         string    `json:"caseId"`
	OwnerIdentity  string    `json:"ownerIdentity"`
	DisclosureRole string    `json:"disclosureRole"`
	PackageHash    string    `json:"packageHash"`
	PackageData    JSONB     `json:"packageData,omitempty"`
	Signature      string    `json:"signature"`
	CreatedAt      time.Time `json:"createdAt"`
}

type AbuseDisclosureRequest struct {
	ID                string    `json:"id"`
	CaseID            string    `json:"caseId"`
