...
func NewS3ObjectStore(config S3ObjectStoreConfig) (*S3ObjectStore, error) {
	...
	if endpoint.Scheme != "https" && !storageDevMode() {
		return nil, errors.New("s3 endpoint must use https outside development mode")
	}
...
func storageDevMode() bool {
	return strings.EqualFold(strings.TrimSpace(os.Getenv("GAIACOM_DEV_MODE")), "true")
}
