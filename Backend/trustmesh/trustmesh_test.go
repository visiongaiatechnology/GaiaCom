...
func TestTrustMeshServiceSubmitReport(t *testing.T) {
	...
	epochSecret[0] = 0xAB // some fixed value
	svc := NewService(store, epochSecret)
...
func TestQuarantineAndTimeoutPenalties(t *testing.T) {
	...
	_, err := msgSvc.SaveAndDistributeMessage(ctx, bobUser.ID, bobIdent.ID, []byte(`{"body":"spam"}`), []uuid.UUID{aliceIdent.ID})
	if err != nil {
	...
	_, err = msgSvc.SaveAndDistributeMessage(ctx, bobUser.ID, bobIdent.ID, []byte(`{"body":"more spam"}`), []uuid.UUID{aliceIdent.ID})
...
func TestSubmitReportHTTP(t *testing.T) {
	...
	if strings.Contains(router.Body.String(), "message inbox entry not found") {
		t.Errorf("Response leaked internal validation error: %s", router.Body.String())
	}
	if !strings.Contains(router.Body.String(), "Trust report rejected") {
		t.Errorf("Expected opaque trust report rejection, got %s", router.Body.String())
	}
}
