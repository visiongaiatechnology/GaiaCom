...
const maxDisclosurePackageBytes = 256 * 1024
const defaultGaiasEyesTTL = 15 * time.Minute
const maxGaiasEyesTTL = 24 * time.Hour
...
func stableBytesHash(value []byte) string {
	sum := sha256.Sum256(value)
	return fmt.Sprintf("sha256:%x", sum[:])
}

func tokenHash(token string) string {
	sum := sha256.Sum256([]byte("gaias-eyes:v1:" + token))
	return fmt.Sprintf("sha256:%x", sum[:])
}
...
func validateDisclosurePackage(packageData json.RawMessage) ([]byte, error) {
	trimmed := []byte(strings.TrimSpace(string(packageData)))
	if len(trimmed) == 0 {
		return nil, errors.New("disclosure package required")
	}
	if len(trimmed) > maxDisclosurePackageBytes {
		return nil, errors.New("disclosure package too large")
	}
	if !json.Valid(trimmed) {
		return nil, errors.New("invalid disclosure package")
	}
	if len(trimmed) == 0 || trimmed[0] != '{' {
		return nil, errors.New("disclosure package must be a JSON object")
	}
	return trimmed, nil
}

func normalizeDisclosureRole(role string) (string, error) {
	switch canonicalReportField(role) {
	case "reporter", "reported", "owner", "authority", "other":
		return canonicalReportField(role), nil
	default:
		return "", errors.New("invalid disclosure role")
	}
}

func normalizeDisclosureScope(scope string) (string, error) {
	switch canonicalReportField(scope) {
	case "", "case", "message", "mail", "file", "profile":
		if canonicalReportField(scope) == "" {
			return "case", nil
		}
		return canonicalReportField(scope), nil
	default:
		return "", errors.New("invalid disclosure scope")
	}
}

func normalizeDisclosureTTL(ttlSeconds int64) time.Duration {
	if ttlSeconds <= 0 {
		return defaultGaiasEyesTTL
	}
	duration := time.Duration(ttlSeconds) * time.Second
	if duration > maxGaiasEyesTTL {
		return maxGaiasEyesTTL
	}
...
func (s *Service) canAttachDisclosure(ctx context.Context, actorGaiaID string, c *models.AbuseCase) bool {
	if c == nil {
		return false
	}
	if c.ReporterIdentityHash == reporterIdentityHash(actorGaiaID) || c.ReporterIdentityHash == legacyReporterIdentityHash(actorGaiaID) {
		return true
	}
	if strings.EqualFold(strings.TrimSpace(c.ReportedIdentityHash), strings.TrimSpace(actorGaiaID)) {
		return true
	}
	for _, role := range []string{"trusted_reviewer", "senior_reviewer", "node_operator"} {
		if has, _ := s.HasRole(ctx, actorGaiaID, role); has {
			return true
		}
	}
	return false
...
func formatDisclosureTime(value time.Time) string {
	if value.IsZero() {
		return ""
	}
...
func (s *Service) CreateCaseDisclosure(ctx context.Context, actorGaiaID string, caseID string, role string, packageData json.RawMessage, signature string) (*models.AbuseDisclosurePackage, error) {
	...
	if !s.canAttachDisclosure(ctx, actorGaiaID, c) {
		return nil, errors.New("disclosure not authorized")
	}
	normalizedRole, err := normalizeDisclosureRole(role)
	if err != nil {
		return nil, err
	}
	cleanPackage, err := validateDisclosurePackage(packageData)
	if err != nil {
		return nil, err
	}
	signature = strings.TrimSpace(signature)
	if len(signature) > 4096 {
		return nil, errors.New("disclosure signature too large")
	}
	pkg := &models.AbuseDisclosurePackage{
		ID:             "disc_" + uuid.New().String(),
		CaseID:         caseID,
		OwnerIdentity:  actorGaiaID,
		DisclosureRole: normalizedRole,
		PackageHash:    stableBytesHash(cleanPackage),
		PackageData:    models.JSONB(cleanPackage),
		Signature:      signature,
		CreatedAt:      time.Now().UTC(),
	}
	if err := s.store.CreateAbuseDisclosurePackage(ctx, pkg); err != nil {
		return nil, err
	}
	_ = s.store.CreateAbuseCaseEvent(ctx, &models.AbuseCaseEvent{
		CaseID:        caseID,
		EventType:     "disclosure_attached",
		ActorIdentity: actorGaiaID,
		Details:       fmt.Sprintf("Voluntary %s disclosure package attached with hash %s.", normalizedRole, pkg.PackageHash),
		Timestamp:     pkg.CreatedAt,
	})
	return pkg, nil
}

func (s *Service) RequestCaseDisclosure(ctx context.Context, requesterGaiaID string, caseID string, targetRole string, reason string) (*models.AbuseDisclosureRequest, error) {
	caseID = strings.TrimSpace(caseID)
	if caseID == "" {
		return nil, errors.New("case id required")
	}
	...
	allowed := false
	for _, role := range []string{"trusted_reviewer", "senior_reviewer", "node_operator"} {
		if has, _ := s.HasRole(ctx, requesterGaiaID, role); has {
			allowed = true
			break
		}
	}
	if !allowed {
		return nil, errors.New("disclosure request requires reviewer or node operator role")
	}
	normalizedRole, err := normalizeDisclosureRole(targetRole)
	if err != nil {
		return nil, err
	}
	reason = strings.TrimSpace(reason)
	if reason == "" {
		return nil, errors.New("disclosure request reason required")
	}
	if len([]rune(reason)) > 1200 {
		return nil, errors.New("disclosure request reason too large")
	}
	now := time.Now().UTC()
	req := &models.AbuseDisclosureRequest{
		ID:                "dreq_" + uuid.New().String(),
		CaseID:            caseID,
		RequesterIdentity: requesterGaiaID,
	...
	if err := s.store.CreateAbuseDisclosureRequest(ctx, req); err != nil {
		return nil, err
	}
	_ = s.store.CreateAbuseCaseEvent(ctx, &models.AbuseCaseEvent{
		CaseID:        caseID,
		EventType:     "disclosure_requested",
		ActorIdentity: requesterGaiaID,
		Details:       fmt.Sprintf("Disclosure requested from %s: %s", normalizedRole, reason),
		Timestamp:     now,
	})
	return req, nil
}

func (s *Service) CreateGaiasEyesGrant(ctx context.Context, ownerGaiaID string, caseID string, scope string, packageData json.RawMessage, signature string, ttlSeconds int64) (*models.GaiasEyesGrant, string, error) {
	normalizedScope, err := normalizeDisclosureScope(scope)
	if err != nil {
		return nil, "", err
	}
	caseID = strings.TrimSpace(caseID)
	if caseID != "" {
		...
		if !s.canAttachDisclosure(ctx, ownerGaiaID, c) {
			return nil, "", errors.New("grant not authorized for case")
		}
	}
	cleanPackage, err := validateDisclosurePackage(packageData)
	if err != nil {
	...
	signature = strings.TrimSpace(signature)
	if len(signature) > 4096 {
		return nil, "", errors.New("grant signature too large")
	}
	...
	grant := &models.GaiasEyesGrant{
		ID:            "eyes_" + uuid.New().String(),
		TokenHash:     tokenHash(token),
		OwnerIdentity: ownerGaiaID,
		CaseID:        caseID,
		...
		Signature:     signature,
		Status:        "active",
		ExpiresAt:     now.Add(normalizeDisclosureTTL(ttlSeconds)),
		UsedAt:        "",
		CreatedAt:     now,
	}
	if err := s.store.CreateGaiasEyesGrant(ctx, grant); err != nil {
		return nil, "", err
	}
	if caseID != "" {
		_ = s.store.CreateAbuseCaseEvent(ctx, &models.AbuseCaseEvent{
			CaseID:        caseID,
			EventType:     "gaias_eyes_key_issued",
			ActorIdentity: ownerGaiaID,
			Details:       fmt.Sprintf("One-time disclosure key issued for scope %s with hash %s.", normalizedScope, grant.PackageHash),
			Timestamp:     now,
		})
	}
...
func (s *Service) ConsumeGaiasEyesGrant(ctx context.Context, token string) (*models.GaiasEyesGrant, error) {
	token = strings.TrimSpace(token)
	if token == "" || len(token) > 256 {
		return nil, errors.New("invalid disclosure token")
	}
	grant, err := s.store.GetGaiasEyesGrantByTokenHash(ctx, tokenHash(token))
	if err != nil || grant == nil {
		return nil, errors.New("disclosure token not found")
	}
	now := time.Now().UTC()
	if grant.Status != "active" || strings.TrimSpace(grant.UsedAt) != "" {
		return nil, errors.New("disclosure token already used")
	}
	if !grant.ExpiresAt.IsZero() && now.After(grant.ExpiresAt) {
		return nil, errors.New("disclosure token expired")
	}
	if err := s.store.MarkGaiasEyesGrantUsed(ctx, grant.ID, now); err != nil {
		return nil, errors.New("disclosure token already used")
	}
	grant.Status = "used"
	grant.UsedAt = formatDisclosureTime(now)
	if grant.CaseID != "" {
		_ = s.store.CreateAbuseCaseEvent(ctx, &models.AbuseCaseEvent{
			CaseID:        grant.CaseID,
			EventType:     "gaias_eyes_key_consumed",
			ActorIdentity: "external-disclosure-viewer",
			Details:       fmt.Sprintf("One-time disclosure key consumed for package hash %s.", grant.PackageHash),
			Timestamp:     now,
		})
	}
	return grant, nil
