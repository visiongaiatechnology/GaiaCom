...
func (h *Handler) GetReportDetail(w http.ResponseWriter, r *http.Request) {
	...
	if !isStaff {
		disclosures, _ := h.Service.store.GetAbuseDisclosurePackages(r.Context(), caseID)
		disclosureRequests, _ := h.Service.store.GetAbuseDisclosureRequests(r.Context(), caseID)
...
func (h *Handler) CreateCaseDisclosure(w http.ResponseWriter, r *http.Request) {
	userID, ok := auth.UserIDFromContext(r.Context())
	if !ok {
		httpx.WriteError(w, http.StatusUnauthorized, "Unauthorized")
		return
	...
	var input disclosureInput
	r.Body = http.MaxBytesReader(w, r.Body, maxDisclosurePackageBytes+8192)
	decoder := json.NewDecoder(r.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil {
		httpx.WriteError(w, http.StatusBadRequest, "Invalid disclosure payload")
		return
	}
	identityUUID, err := uuid.Parse(input.IdentityID)
	if err != nil {
	...
	pkg, err := h.Service.CreateCaseDisclosure(r.Context(), identity.GaiaID, caseID, input.Role, input.Package, input.Signature)
	if err != nil {
		writeGovernanceRejection(w, "disclosure", err)
		return
	}
	httpx.WriteJSON(w, http.StatusCreated, pkg)
}

func (h *Handler) RequestCaseDisclosure(w http.ResponseWriter, r *http.Request) {
	userID, ok := auth.UserIDFromContext(r.Context())
	if !ok {
		httpx.WriteError(w, http.StatusUnauthorized, "Unauthorized")
		return
	...
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil {
		httpx.WriteError(w, http.StatusBadRequest, "Invalid disclosure request payload")
		return
	}
	identityUUID, err := uuid.Parse(input.IdentityID)
	if err != nil {
	...
	req, err := h.Service.RequestCaseDisclosure(r.Context(), identity.GaiaID, caseID, input.TargetRole, input.Reason)
	if err != nil {
		writeGovernanceRejection(w, "disclosure-request", err)
		return
	}
	httpx.WriteJSON(w, http.StatusCreated, req)
}

func (h *Handler) CreateGaiasEyesGrant(w http.ResponseWriter, r *http.Request) {
	userID, ok := auth.UserIDFromContext(r.Context())
	if !ok {
		httpx.WriteError(w, http.StatusUnauthorized, "Unauthorized")
		return
	...
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil {
		httpx.WriteError(w, http.StatusBadRequest, "Invalid GaiasEyesKey payload")
		return
	}
	identityUUID, err := uuid.Parse(input.IdentityID)
	if err != nil {
	...
	grant, token, err := h.Service.CreateGaiasEyesGrant(r.Context(), identity.GaiaID, input.CaseID, input.Scope, input.Package, input.Signature, input.TTLSeconds)
	if err != nil {
		writeGovernanceRejection(w, "gaias-eyes-issue", err)
		return
	}
...
func (h *Handler) ConsumeGaiasEyesGrant(w http.ResponseWriter, r *http.Request) {
	...
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil {
		httpx.WriteError(w, http.StatusBadRequest, "Invalid GaiasEyesKey")
		return
	}
	if input.Type != "GaiasEyesKey" || input.Version != "v1" {
		httpx.WriteError(w, http.StatusBadRequest, "Unsupported GaiasEyesKey")
		return
	}
	grant, err := h.Service.ConsumeGaiasEyesGrant(r.Context(), input.Token)
	if err != nil {
		writeGovernanceRejection(w, "gaias-eyes-consume", err)
		return
	}
	httpx.WriteJSON(w, http.StatusOK, map[string]interface{}{
		"status":  "consumed",
