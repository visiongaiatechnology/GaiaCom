...
func TestProductionSecuritySystemRequiresStrongSecret(t *testing.T) {
	t.Setenv("GAIACOM_DEV_MODE", "false")
	for _, secret := range []string{"", "short"} {
		t.Setenv("GAIACOM_SHIELD_SECRET", secret)
		if system, err := NewSecuritySystem(NewMockStore()); err == nil || system != nil {
			t.Fatalf("accepted weak production shield secret %q", secret)
		}
	}
}
...
func TestEdgeShieldMiddleware_Traversal(t *testing.T) {
	store := NewMockStore()
	sys, err := NewSecuritySystem(store)
	if err != nil {
		t.Fatal(err)
	}
	sys.Store = store
