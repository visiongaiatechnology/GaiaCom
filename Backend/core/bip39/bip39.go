...
// IsCanonicalMnemonicBytesValid validates the canonical UTF-8 representation
// used by the GaiaCom web client: English BIP-39 words separated by exactly one
// U+0020 byte. The byte API avoids creating an immutable Go string containing
// recovery material on native mobile clients.
func IsCanonicalMnemonicBytesValid(mnemonic []byte) bool {
	entropy, err := canonicalMnemonicBytesToEntropy(mnemonic)
	wipeBytes(entropy)
	return err == nil
}

// NewSeedFromCanonicalBytes derives a BIP-39 seed without converting recovery
// material into an immutable Go string. Callers must validate the mnemonic and
// provide already NFKD-normalized bytes. GaiaCom's English wordlist is ASCII,
// so every accepted canonical mnemonic is already in NFKD form.
func NewSeedFromCanonicalBytes(mnemonic, passphrase []byte) []byte {
	salt := make([]byte, len("mnemonic")+len(passphrase))
	copy(salt, "mnemonic")
	copy(salt[len("mnemonic"):], passphrase)
	defer wipeBytes(salt)
	return pbkdf2Key(mnemonic, salt, 2048, 64)
}
...
func canonicalMnemonicBytesToEntropy(mnemonic []byte) ([]byte, error) {
	words, ok := splitCanonicalWords(mnemonic)
	if !ok || len(words) < 12 || len(words) > 24 || len(words)%3 != 0 {
		return nil, ErrInvalidMnemonic
	}
	...
	defer wipeBytes(stream)
	for index, word := range words {
		wordID, found := wordIndexBytes(word)
		if !found {
			return nil, ErrInvalidMnemonic
		}
		write11Bits(stream, index*11, wordID)
	}

	entropy := make([]byte, entropyBits/8)
	copy(entropy, stream[:len(entropy)])
	hash := sha256.Sum256(entropy)
	defer wipeBytes(hash[:])
	expected := hash[0] >> (8 - uint(checksumBits))
	actual := readNBits(stream, entropyBits, checksumBits)
	if subtle.ConstantTimeByteEq(expected, actual) != 1 {
		wipeBytes(entropy)
		return nil, ErrInvalidMnemonic
	}
	return entropy, nil
...
func splitCanonicalWords(mnemonic []byte) ([][]byte, bool) {
	if len(mnemonic) == 0 || mnemonic[0] == ' ' || mnemonic[len(mnemonic)-1] == ' ' {
		return nil, false
	}
	words := make([][]byte, 0, 24)
	start := 0
	for index, value := range mnemonic {
		switch {
		case value == ' ':
			if index == start {
				return nil, false
			}
			words = append(words, mnemonic[start:index])
			start = index + 1
...
func wordIndexBytes(word []byte) (int, bool) {
	for index, candidate := range englishWordlist {
		if len(candidate) != len(word) {
			continue
		}
		matches := true
		for byteIndex := range word {
			if candidate[byteIndex] != word[byteIndex] {
				matches = false
				break
			}
		}
...
func hmacSHA512(key, data []byte) []byte {
	...
	defer wipeBytes(ipad)
	defer wipeBytes(opad)
	copy(ipad, key)
	copy(opad, key)
	wipeBytes(keyHash[:])
	for i := range ipad {
