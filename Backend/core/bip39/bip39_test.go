...
func TestCanonicalMnemonicByteAPI(t *testing.T) {
	mnemonic := []byte("abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about")
	if !IsCanonicalMnemonicBytesValid(mnemonic) {
		t.Fatal("canonical byte mnemonic was rejected")
	}
	for _, invalid := range [][]byte{
		append([]byte(" "), mnemonic...),
		append(append([]byte(nil), mnemonic...), ' '),
		[]byte(strings.Replace(string(mnemonic), "abandon abandon", "abandon  abandon", 1)),
		[]byte(strings.Replace(string(mnemonic), "abandon abandon", "abandon\tabandon", 1)),
	} {
		if IsCanonicalMnemonicBytesValid(invalid) {
			t.Fatal("non-canonical byte mnemonic was accepted")
		}
	}
	seed := NewSeedFromCanonicalBytes(mnemonic, nil)
	defer wipeBytes(seed)
	if len(seed) != 64 || hex.EncodeToString(seed) != hex.EncodeToString(NewSeed(string(mnemonic), "")) {
		t.Fatal("canonical byte seed differs from the string BIP-39 implementation")
	}
}
